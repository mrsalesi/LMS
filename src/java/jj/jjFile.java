 ...
 */
public class jjFile {
     private static String Save_Folder_Name = "/upload"; //=============mirhaj
    ///\:*?"<>|
    ...
    //====================================>mirhaj
    //تابعی برای حذف کردن فایل ها و عکس ها از فایل آپلود
    //لازم است نام فایل یا عکس در 
    //Request
    //قرار گیرد و سپس این تابع فراخوانی شود
    //به این صورت:
    //request.setAttribute("fileName", نام فایل);
    
    public static boolean deletefile(HttpServletRequest request, jjDatabaseWeb db, boolean isPost) throws Exception {
        try {
            
            int index = request.getServletContext().getRealPath("/").indexOf("build");
            ...
            File file = new File(folderAddress + "/" + filename);
            System.out.println("filename= " +file);
            if (filename.contains(".jpg" )) {
                File file2 = new File(folderAddress + "/" + filename.replace(".", "_small."));
                file2.delete();
            }
